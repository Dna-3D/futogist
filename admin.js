...
function generateFormFields(type, data = {}) {
    ...
    switch(type) {
        ...
        case 'polls':
            const optionsHtml = data.options ? data.options.map((option, index) => `
                <div class="form-group">
                    <label for="option${index}">Option ${index + 1}</label>
                    <input type="text" id="option${index}" name="option${index}" value="${option.text || ''}" required>
                </div>
            `).join('') : `
                <div class="form-group">
                    <label for="option0">Option 1</label>
                    <input type="text" id="option0" name="option0" required>
                </div>
                <div class="form-group">
